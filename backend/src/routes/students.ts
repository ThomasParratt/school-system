...
router.get("/", async (req, res) => {
  try {
    const result = await pool.query("SELECT id, name, first_name, second_name, email FROM students");
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      "INSERT INTO students (name, first_name, second_name, email, password) VALUES ($1, $2, $3, $4, $5) RETURNING id, name, first_name, second_name, email",
